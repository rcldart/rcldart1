...
String _getObjectFilename() {
  ...
  return 'librcl.$extension';
}

/// TensorFlowLite C library.
DynamicLibrary rcllib = () {
  var objectFile;
  if (Platform.script.path.endsWith('.snapshot')) {
    // If we're running from snapshot, assume that the shared object
    // file is a sibling.
    objectFile = File.fromUri(Platform.script).parent.path + '/' + _getObjectFilename();
  } else {
    final rootLibrary = 'package:rcldart/rcllib.dart';
    final blobs = cli.waitFor(Isolate.resolvePackageUri(Uri.parse(rootLibrary))).resolve('src/blobs/');
